...
def is_picture(filename):
    """Check if the file given is a picture file.

    It is determined by the extension of the file

    Args:
        filename (str): path to a file and filename

    Returns (boolean):
        True if the file is a picture file, otherwise None
    """

    ext = splitext(filename)[1][1:].upper()
    return ext in [ext.upper() for ext in ADDON.getSetting("picsext").split("|")]


def is_video(filename):
    """Check if the file given is a video file.
...
def find_fanart(path, filename):
    """find fanart file of the filename given.
...
def find_folder_thumb(folderpath):
...
# TODO when?
def find_cached_thumb_db(folderpath):
    """
