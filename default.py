...
class Main:
    ...
    def get_args(self):
        ...
        self.parm = common.smart_utf8(
            unquote(sys.argv[2])).replace("\\\\", "\\")
    ...
    def add_directory(self, name, params, action, iconimage=None, fanart=None,
        ...
        try:
            ...
            if path:
                # Use path instead of plugin call. In this way, we can use the default folder icon generated by Kodi.
                u = path
            else:
                u = sys.argv[0] + "?" + parameter + "&action=" + \
                    repr(str(action)) + "&name=" + \
                    repr(common.quote_param(name))
    ...
    def add_action(self, name, params, action, iconimage, fanart=None, contextmenu=None, total=0, info="*",
                   replacemenu=True):
        try:
            try:
                parameter = "&".join(
            ...
            u = sys.argv[0] + "?" + parameter + "&action=" + repr(str(action)) + "&name=" + repr(
    ...
    def add_picture(self, picname, picpath, count=0, info="*", fanart=None, contextmenu=None, replacemenu=True):
        ...
        try:
            ...
            # is the file a video ?
            if extension in ["." + ext.replace(".", "").upper() for ext in
                             common.getaddon_setting("vidsext").split("|")]:

                infolabels = {"date": date}
                liz.setInfo(type="video", infoLabels=infolabels)
            # or is the file a picture ?
            elif extension in ["." + ext.replace(".", "").upper() for ext in
                               common.getaddon_setting("picsext").split("|")]:
                ...
                if date is None:
                    infolabels = {"picturepath": picname +
                                  " " + suffix, "count": count}
                else:
                    infolabels = {"picturepath": picname +
                ...
                if int(rating) > 0:
                    ...
                    suffix = suffix + "[COLOR=C0FFFF00]" + ("*" * int(rating)) + "[/COLOR][COLOR=C0C0C0C0]" + (
                        "*" * (5 - int(rating))) + "[/COLOR]"
            ...
            #        contextmenu.append( (common.getstring(30220),"RunPlugin(\"%s?action='geolocate'&place='%s'&path='%s'&filename='%s'&viewmode='view'\" ,)"%(sys.argv[0],"%0.6f,%0.6f"%(coords),
            #                                                                                                                                                   common.quote_param(picpath),
            #                                                                                                                                                   common.quote_param(picname)
            #                                                                                                                                                   )))
    ...
    def change_view(self):
        ...
        if skin_dir in view_modes:
            xbmc.executebuiltin(
                'Container.SetViewMode(' + str(view_modes[skin_dir]) + ')')
    ...
    def show_home(self):
        ...
        if common.getaddon_setting('m_1') == 'true' or display_all:
            self.add_directory(common.getstring(30209) % common.getaddon_setting("recentnbdays"),
                               [("method", "recentpicsdb"), ("period", ""), ("value", ""), ("page", "1"),("viewmode", "view")],
                               "showpics", 
                               join(PIC_PATH, "folder_recent_added.png"))

        # Last pictures
        if common.getaddon_setting('m_2') == 'true' or display_all:
            self.add_directory(common.getstring(30130) % common.getaddon_setting("lastpicsnumber"),
                               [("method", "lastpicsshooted"), ("page", "1"), ("viewmode", "view")],
                               "showpics", 
                               join(PIC_PATH, "folder_recent_shot.png"))

        # N random pictures
        if common.getaddon_setting('m_13') == 'true' or display_all:
            self.add_directory(common.getstring(30654) % common.getaddon_setting("randompicsnumber"),
                               [("method", "random"), ("page", "1"), ("viewmode", "view")],
                               "showpics", 
                               join(PIC_PATH, "folder_random.png"))

        # videos
        if common.getaddon_setting('m_3') == 'true' or display_all and common.getaddon_setting("usevids") == "true":
            self.add_directory(common.getstring(30051),
                               [("method", "videos"), ("page", "1"), ("viewmode", "view")],
                               "showpics", 
                               join(PIC_PATH, "folder_videos.png"))
        ...
        if common.getaddon_setting('m_4') == 'true' or display_all:
            self.add_directory(common.getstring(30101),
                               [("period", "year"), ("value", ""), ("viewmode", "view")],
                               "showdate", 
                               join(PIC_PATH, "folder_date.png"))

        # Browse by Folders
        if common.getaddon_setting('m_5') == 'true' or display_all:
            self.add_directory(common.getstring(30102),
                               [("method", "folders"), ("folderid", ""), ("onlypics", "non"), ("viewmode", "view")],
                               "showfolder", 
                               join(PIC_PATH, "folder_pictures.png"))

        # Browse by root folders only
        if common.getaddon_setting('m_6') == 'true' or display_all:
            self.add_directory(common.getstring(30103),
                               [("method", "folders"), ("folderid", "root"), ("onlypics", "non"), ("viewmode", "view")],
                               "showfolder", 
                               join(PIC_PATH, "folder_pictures.png"))

        # Browse by child folders only
        if common.getaddon_setting('m_7') == 'true' or display_all:
            self.add_directory(common.getstring(30104),
                               [("method", "folders"), ("folderid", "child"), ("onlypics", "non"), ("viewmode", "view")],
                               "showfolder", 
                               join(PIC_PATH, "folder_pictures.png"))

        # Browse by Tags
        if common.getaddon_setting('m_14') == 'true' or display_all:
            self.add_directory(common.getstring(30122),
                               [("tags", ""), ("viewmode", "view")], 
                               "showtagtypes", 
        ...
        if common.getaddon_setting('m_10') == 'true' or display_all:
            self.add_directory(common.getstring(30105),
                               [("period", ""), ("viewmode", "view"), ], 
                               "showperiod", 
                               join(PIC_PATH, "folder_date_ranges.png"))

        # Collections
        if common.getaddon_setting('m_11') == 'true' or display_all:
            self.add_directory(common.getstring(30150),
                               [("collect", ""), ("method", "show"), ("viewmode", "view")],
                               "showcollection", 
                               join(PIC_PATH, "folder_collections.png"))

        # Global search
        if common.getaddon_setting('m_12') == 'true' or display_all:
            self.add_directory(common.getstring(30098),
                               [("searchterm", ""), ("viewmode", "view")], 
                               "globalsearch",
        ...
        self.add_action(common.getstring(30620),
                        [("showtranslationeditor", ""), ("viewmode", "view")], "showtranslationeditor",
        ...
        # xbmcplugin.setPluginCategory( handle=int( sys.argv[ 1 ] ), category=unquote("My Pictures Library".encode("utf-8")) )
    ...
    def show_date(self):
        ...
        dptd = dptd.replace("%b", monthname[
            strptime(self.args.value, thisdateformat).tm_mon - 1])  # replace %b marker by short month name
        dptd = dptd.replace("%B", fullmonthname[
            strptime(self.args.value, thisdateformat).tm_mon - 1])  # replace %B marker by long month name
        ...
        if count > 0:
            self.add_directory(name=common.getstring(30100) % (nameperiode, count),
                               # libellé#"All the period %s (%s pics)"%(self.args.value,MPDB.count_pics_in _period(allperiod,self.args.value)), #libellé
                               ...
                               contextmenu=[(common.getstring(30152),
                                             "RunPlugin(\"%s?action='addfolder'&method='date'&period='%s'&value='%s'&viewmode='scan'\")" % (
        ...
        if count > 0 and self.args.period == "year":
            self.add_directory(name=common.getstring(30054) % (count),
                               params=[("method", "date"), ("period", "wo"), ("value", self.args.value), ("page", ""),
                                       ("viewmode", "view")],
                               # paramètres
                               action="showpics",  # action
                               iconimage=join(
            ...
            if period:
                if action == "showpics":
                    context = [(common.getstring(30152),
                                "RunPlugin(\"%s?action='addfolder'&method='date'&period='%s'&value='%s'&page=''&viewmode='scan'\")" % (
                ...
                try:
                    ...
                    self.add_directory(name="%s (%s %s)" % (strftime(self.prettydate(displaydate, dateformat), dateformat),
                                       ...
                                       params=[("method", "date"), ("period", nextperiod), ("value", period), ("viewmode", "view")],
    ...
    def show_folders(self):
        ...
        if not self.args.folderid:  # No Id given, get all folders
            ...
            # apply fixed icon image only to root folders, i.e. top most picture source.
            # for other folders, use the one kodi creates, which is composed of 4 thumbnails of pics inside.
        ...
        for idchildren, childrenfolder in childrenfolders:
            ...
            if count > 0:
                self.add_directory(name="%s (%s %s)" % (childrenfolder, count, common.getstring(30050)),  # libellé
                                   params=[("method", "folders"), ("folderid", str(idchildren)), ("onlypics", "non"), ("viewmode", "view")],
                                   action="showfolder",  # action
                                   iconimage=iconimage,  # icone
                                   contextmenu=[(common.getstring(30212),
                                                 "Container.Update(\"%s?action='rootfolders'&do='addrootfolder'&addpath='%s'&exclude='1'&viewmode='view'\",)" % (
        ...
        if min_rating > 0:
            picsfromfolder = [row for row in MPDB.cur.request_with_binds(
                "SELECT p.FullPath, f.strFilename FROM Files f, Folders p WHERE f.idFolder=p.idFolder AND f.idFolder=? AND f.ImageRating > ? order by f.imagedatetime",
                (self.args.folderid, min_rating,))]
        else:
            picsfromfolder = [row for row in MPDB.cur.request_with_binds(
                "SELECT p.FullPath, f.strFilename FROM Files f, Folders p WHERE f.idFolder=p.idFolder AND f.idFolder=? order by f.imagedatetime",
                (self.args.folderid,))]
        ...
        for path, filename in picsfromfolder:
            ...
            context.append((common.getstring(30152),
                            "RunPlugin(\"%s?action='addtocollection'&viewmode='view'&path='%s'&filename='%s'\")" % (
                                sys.argv[0],
    ...
    def show_wizard(self):
        if self.args.wizard == 'dialog':
            ...
            if len(GlobalFilterTrue) > 0 or len(GlobalFilterFalse) > 0 or start_date != '' or end_date != '':
                xbmc.executebuiltin(
                    "Container.Update(%s?action='showpics'&viewmode='view'&method='wizard'&matchall='%s'&kw='%s'&nkw='%s'&start='%s'&end='%s')" % (
                        sys.argv[0], matchall, common.quote_param(
                            newtagtrue), common.quote_param(newtagfalse),
                        start_date,
                        end_date))
            ...
            for filtername in filterlist:
                ...
                self.add_directory(name="%s" % (filtername),
                                   params=[("method", "wizard_settings"), ("viewmode", "view"),
                                           ("filtername", filtername), ("period", ""), ("value", ""), ("page", "1")],
                                   action="showpics",
                                   iconimage=join(
                                       PIC_PATH, "folder_wizard.png"),
                                   contextmenu=[(common.getstring(30152),
                                                 "RunPlugin(\"%s?action='addfolder'&method='wizard_settings'&filtername='%s'&viewmode='scan'\")" % (
                                                     sys.argv[0], filtername)), ],
                                   # contextmenu   = [('','')],
                                   total=total)
            xbmcplugin.addSortMethod(
        ...
        for tag, nb in listtags:
            if nb:
                self.add_directory(name="%s (%s %s)" % (tag, nb, common.getstring(30050)),  # libellé
                                   ...
                                   action="showpics",  # action
                                   iconimage=join(
                                       PIC_PATH, "folder_tags.png"),  # icone
                                   contextmenu=[(common.getstring(30152),
                                                 "RunPlugin(\"%s?action='addfolder'&method='tag'&tag='%s'&tagtype='%s'&viewmode='scan'\")" % (
                                                     sys.argv[0], common.quote_param(tag), tagtype)),
                                                (common.getstring(30061),
                                                 "RunPlugin(\"%s?action='showpics'&method='tag'&page=''&viewmode='zip'&name='%s'&tag='%s'&tagtype='%s'\")" % (
                                                     sys.argv[0], common.quote_param(
                                                         tag), common.quote_param(tag),
                                                     tagtype)),
                                                (common.getstring(30062),
                                                 "RunPlugin(\"%s?action='showpics'&method='tag'&page=''&viewmode='export'&name='%s'&tag='%s'&tagtype='%s'\")" % (
                                                     sys.argv[0], common.quote_param(
                                                         tag), common.quote_param(tag),
                                                     tagtype))
                                                ],  # menucontextuel
    ...
    def show_period(self):  # TODO finished the datestart and dateend editing
        ...
        if self.args.period == "setperiod":
            ...
            if len(nameddates):
                ...
                rets = dialog.select(common.getstring(30107), [
                    "[[%s]]" % common.getstring(30114)] + nameddates)  # dateofpics)#choose the start date
                if not rets == -1:  # is not canceled
                    ...
                    if datestart != '':
                        retf = dialog.select(common.getstring(30108), ["[[%s]]" % common.getstring(30114)] + nameddates[
                            deb:])  # dateofpics[deb:])#choose the end date (all dates before startdate are ignored to preserve begin/end)
                ...
                common.log("show_period",
                           "No pictures with an EXIF date stored in DB")
        ...
        for periodname, dbdatestart, dbdateend in MPDB.periods_list():
            ...
            datestart, dateend = MPDB.period_dates_get_pics(
                dbdatestart, dbdateend)
            ...
            self.add_directory(name="%s [COLOR=C0C0C0C0](%s)[/COLOR]" % (periodname,
                                                                         common.getstring(30113) % (strftime(
                                                                             self.prettydate(common.getstring(30002),
                                                                                             strptime(datestart,
                                                                                                      "%Y-%m-%d")),
                                                                             strptime(datestart, "%Y-%m-%d")),
                                                                             strftime(
                                                                             self.prettydate(
                                                                                 common.getstring(
                                                                                     30002),
                               ...
                               params=[("method", "date"), ("period", "period"), ("datestart", datestart),
                                       ("dateend", dateend), ("page", "1"), ("viewmode", "view")],
                               # paramètres
                               action="showpics",  # action
                               iconimage=join(
                                   PIC_PATH, "folder_date_ranges.png"),  # icone
                               contextmenu=[(common.getstring(30111),
                                             "RunPlugin(\"%s?action='removeperiod'&viewmode='view'&periodname='%s'&period='period'\")" % (
                                                 sys.argv[0], common.quote_param(periodname))),
                                            (common.getstring(30112),
                                             "RunPlugin(\"%s?action='renameperiod'&viewmode='view'&periodname='%s'&period='period'\")" % (
                                                 sys.argv[0], common.quote_param(periodname))),
                                            (common.getstring(30152),
                                             "RunPlugin(\"%s?action='addfolder'&method='date'&period='period'&datestart='%s'&dateend='%s'&viewmode='scan'\")" % (
                                                 sys.argv[0], datestart, dateend))
                                            ])  # menucontextuel
    ...
    def show_collection(self):
        ...
        if self.args.method == "setcollection":  # ajout d'une collection
            ...
            if ret > -1:
                ...
                if (kb.isConfirmed()):
                    ...
                    rows = MPDB.filterwizard_get_pics_from_filter(
                        filters[ret], 0)
                    ...
                    if rows != None:
                        ...
                        common.log("show_collection", str(filters[ret]) + " is empty and therefore not created.",
                                   xbmc.LOGINFO)
            refresh = True
        ...
        # herve502
            ...
            dialog = xbmcgui.Dialog()
            importfile = dialog.browse(1, common.getstring(
                30162), "files", ".xml", True, False, "")
            if not importfile:
                return
            ...
            try:
                ...
                if (kb.isConfirmed()):
                    ...
                    file_names = album.getElementsByTagName(
                        "itemOriginalPath")  # Xycl get pictures with complete path name
                    for itemName in file_names:  # iterate over the nodes
                        ...
                        try:
                            ...
                            filename, pathname = MPDB.cur.request_with_binds(
                                "select strFilename, strPath from Files where lower(strFilename) = ? and lower(strPath) = ? ",
                            ...
                            try:
                                ...
                                filename, pathname = MPDB.cur.request_with_binds(
                                    "select strFilename, strPath from Files where lower(strFilename) = ? and substr(lower(strPath), 1, length(strPath)-1) = ? ",
                                    (filename.lower(), pathname.lower()))[0]
                                MPDB.collection_add_pic(
                                    collection_name, pathname, filename)

                            except:
                                not_imported += common.getstring(
                                    30166) % (filename, pathname)
                                pass
            ...
            if not_imported != "":
                ...
                viewer.Viewer(header=common.getstring(
                    30167), text=not_imported)
            refresh = True
        # /herve502
        else:
            refresh = False
        ...
        for collection in MPDB.collections_list():
            self.add_action(name=collection[0],
                            params=[("method", "collection"), ("collect", collection[0]), ("page", "1"),
                                    ("viewmode", "slideshow")],
                            # paramètres
                            action="showpics",  # action
                            iconimage=join(PIC_PATH, "folder_collections.png"),  # icone
                            ...
                            contextmenu=[
                                (common.getstring(30169),
                                 "Container.Update(\"%s?action='showpics'&method='collection'&page=''&viewmode='view'&name='%s'&collect='%s'\")" % (
                                     sys.argv[0], common.quote_param(collection[0]),
                                     common.quote_param(collection[0]))),
                                (common.getstring(30149),
                                 "RunPlugin(\"%s?action='collectionaddplaylist'&viewmode='view'&collect='%s'\")" % (
                                     sys.argv[0], common.quote_param(collection[0]))),
                                (common.getstring(30158),
                                 "RunPlugin(\"%s?action='removecollection'&viewmode='view'&collect='%s'\")" % (
                                     sys.argv[0], common.quote_param(collection[0]))),
                                (common.getstring(30159),
                                 "RunPlugin(\"%s?action='renamecollection'&viewmode='view'&collect='%s'\")" % (
                                     sys.argv[0], common.quote_param(collection[0]))),
                                (common.getstring(30061),
                                 "RunPlugin(\"%s?action='showpics'&method='collection'&page=''&viewmode='zip'&name='%s'&collect='%s'\")" % (
                                     sys.argv[0], common.quote_param(collection[0]),
                                     common.quote_param(collection[0]))),
                                (common.getstring(30062),
                                 "RunPlugin(\"%s?action='showpics'&method='collection'&page=''&viewmode='export'&name='%s'&collect='%s'\")" % (
                                     sys.argv[0], common.quote_param(collection[0]), common.quote_param(collection[0])))
            ])  # menucontextuel
        ...
        for tag, _ in listtags:
            ...
            if compte:
                ...
                self.add_directory(name=common.getstring(30116) % (compte, motrecherche, tag),
                                   # files_fields_description.has_key(colname) and files_fields_description[colname] or colname),
                                   params=[("method", "search"), ("field", u"%s" % common.smart_unicode(tag)),
                                           ("searchterm", u"%s" % common.smart_unicode(
                                               motrecherche)), ("page", "1"),
                                           ("viewmode", "view")],
                                   # paramètres
                                   action="showpics",  # action
                                   ...
                                   contextmenu=[(common.getstring(30152),
                                                 "RunPlugin(\"%s?action='addfolder'&method='search'&field='%s'&searchterm='%s'&viewmode='scan'\")" % (
                                                     sys.argv[0], tag, motrecherche))])  # menucontextuel
        if not result:
            dialog = xbmcgui.Dialog()
    ...
    def show_roots(self):
        ...
        if self.args.do == "addroot" or self.args.do == "addpicturessource":  # add a root to scan
                    ...
                    try:
                        if source.startswith('multipath://'):
                            ...
                            for item in newpartialroot:
                                MPDB.add_root_folder(unquote(item), True, True,
                                                     0)  # TODO : traiter le exclude (=0 pour le moment) pour gérer les chemins à exclure
                                common.log(
                                    "Main.show_roots", 'Multipath addroot for part "%s" done' % unquote(item))
                        else:
                            MPDB.add_root_folder(source, True, True,
                                                 0)  # TODO : traiter le exclude (=0 pour le moment) pour gérer les chemins à exclure
                            common.log("Main.show_roots",
                                       'Singlepath addroot "%s" done' % source)

                        xbmc.executebuiltin(
                            "Container.Refresh(\"%s?action='rootfolders'&do='showroots'&exclude='0'&viewmode='view'\",)" % (
                                sys.argv[0],))

                    except:
                        common.log(
                            "Main.show_roots", 'MPDB.add_root_folder failed for "%s"' % source, xbmc.LOGERROR)

                if common.getaddon_setting('scanning') == 'false':
                    common.run_script("%s,--refresh" %
            ...
            if str(self.args.exclude) == "1":
                ...
                xbmc.executebuiltin(
                    "Container.Refresh(\"%s?action='rootfolders'&do='showroots'&exclude='1'&viewmode='view'\",)" % (
                        sys.argv[0],))
                common.log("Main.show_roots",
                           'Exclude folder "%s" added' % newroot)
                # xbmc.executebuiltin( "Notification(%s,%s,%s,%s)"%(common.getstring(30000),common.getstring(30204),3000,join(home,"icon.png") ) )
                dialogok = xbmcgui.Dialog()
                dialogok.ok(common.getstring(30000), common.getstring(
                    30217) + ' ' + common.getstring(30218))
            else:
                recursive = dialog.yesno(common.getstring(30000),
                                         common.getstring(30202)) and 1 or 0  # browse recursively this folder ?
                # dialog.yesno(common.getstring(30000),common.getstring(30203)) and 1 or 0 # Remove files from database if pictures does not exists?
                update = True
                ...
                try:
                    if newroot.startswith('multipath://'):
                        ...
                        for item in newpartialroot:
                            MPDB.add_root_folder(unquote(item), recursive, update,
                                                 0)  # TODO : traiter le exclude (=0 pour le moment) pour gérer les chemins à exclure
                            common.log(
                                "Main.show_roots", 'Multipath addroot for part "%s" done' % unquote(item))
                    else:
                        MPDB.add_root_folder(newroot, recursive, update,
                                             0)  # TODO : traiter le exclude (=0 pour le moment) pour gérer les chemins à exclure
                        common.log("Main.show_roots",
                    ...
                    common.log(
                        "Main.show_roots", 'MPDB.add_root_folder failed for "%s"' % newroot, xbmc.LOGERROR)
                common.show_notification(common.getstring(
                    30000), common.getstring(30204), 3000, join(home, "icon.png"))
        ...
        # I don't think that this is ever called because no user knows about it
            ...
            if str(self.args.exclude) == "1":
                common.log(
                    "Main.show_roots", 'addrootfolder "%s" (exclude) from context menu' % self.args.addpath)
            ...
            try:
                ...
                if dialog.yesno(common.getstring(30250),
                    ...
                    common.log("Main.show_roots", 'delroot "%s"' %
                               self.args.delpath)
                    MPDB.delete_root(self.args.delpath)
                    if self.args.delpath != 'neverexistingpath':
                        common.show_notification(common.getstring(30000), common.getstring(30205), 3000,
                                                 join(home, "icon.png"))
        ...
        if int(sys.argv[1]) >= 0:
            ...
            self.add_action(name=common.getstring(30216),  # add a root path
                            ...
                            iconimage=join(
                                PIC_PATH, "folder_paths.png"),  # icone
                            contextmenu=None)  # menucontextuel

            # Add a path to database
            self.add_action(name=common.getstring(30208),  # add a root path
                            params=[("do", "addroot"), ("viewmode",
                                                        "view"), ("exclude", "0")],  # paramètres
                            action="rootfolders",  # action
                            iconimage=join(
                                PIC_PATH, "folder_paths.png"),  # icone
                            contextmenu=None)  # menucontextuel
            ...
            for path, recursive, update in includefolders:
                ...
                self.add_action(
                    ...
                    contextmenu=[(common.getstring(30206),
                                  "Notification(TODO : scan folder,scan this folder now !,3000,%s)" % join(home,
                                                                                                           "icon.png")),
                                 (common.getstring(30207),
                                  "Container.Update(\"%s?action='rootfolders'&do='delroot'&delpath='%s'&exclude='1'&viewmode='view'\",)" % (
                                      sys.argv[0], common.quote_param(path)))
                                 ]
                )
            ...
            for path, recursive, update in excludefolders:
                self.add_action(name="[COLOR=FFFF0000][B][ - ][/B][/COLOR] " + path,
                                ...
                                contextmenu=[(common.getstring(30210),
                                              "Container.Update(\"%s?action='rootfolders'&do='delroot'&delpath='%s'&exclude='0'&viewmode='view'\",)" % (
                                                  sys.argv[0], common.quote_param(path)))])
    ...
    def collection_add_pic(self):
        ...
        common.show_notification(common.getstring(30000), common.getstring(30154) + ' ' + namecollection, 3000,
                                 join(home, "icon.png"))
        # xbmc.executebuiltin( "Notification(%s,%s %s,%s,%s)"%(common.getstring(30000),common.getstring(30154),namecollection,3000,join(home,"icon.png")))
    ...
    def collection_add_folder(self):
        ...
        common.show_notification(common.getstring(30000),
                                 ...
                                 join(home, "icon.png"))
        # xbmc.executebuiltin( "Notification(%s,%s %s,%s,%s)"%(common.getstring(30000), common.getstring(30161)%len(filelist),namecollection, 3000,join(home,"icon.png")) )
    ...
    def collection_delete(self):
        ...
        if dialog.yesno(common.getstring(30150), common.getstring(30251) % self.args.collect):
            ...
            xbmc.executebuiltin(
                "Container.Update(\"%s?action='showcollection'&viewmode='view'&collect=''&method='show'\" , replace)" %
                sys.argv[0], )
    ...
    def collection_del_pic(self):
        ...
        xbmc.executebuiltin(
            "Container.Update(\"%s?action='showpics'&viewmode='view'&page='1'&collect='%s'&method='collection'\" , replace)" % (
                sys.argv[0], common.quote_param(self.args.collect)), )
    ...
    def set_properties(self):
        ...
        Categories = MPDB.cur.request(
            """select count(distinct tf.idFile) from TagTypes tt, TagContents tc, TagsInFiles tf where tt.idTagType = tc.idTagType and tc.idTagContent = tf.idTagContent and tt.TagTranslation = ( select TagTranslation from TagTypes tti where tti.TagType = 'Category')""")[
            0]
    ...
    def show_pics(self):
        ...
        # we are showing pictures for a RANDOM selection
            ...
            try:
                count = [row for row in MPDB.cur.request(
                    """SELECT count(*) FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ?""",
                    (min_rating,))][0][0]
            except:
                count = 0
            ...
            if MPDB.db_backend.lower() == 'mysql':
                filelist = [row for row in MPDB.cur.request(
                    """SELECT strPath, strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? ORDER BY RAND() LIMIT %s OFFSET %s""" % (
                        limit, offset), (min_rating,))]
            else:
                if count < limit:
                    select = """SELECT strPath, strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= '%s' ORDER BY RANDOM() LIMIT %s OFFSET %s""" % (
                        min_rating, limit, offset)
                else:
                    select = """SELECT strPath, strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= '%s' AND RANDOM() %% %s ORDER BY RANDOM() LIMIT %s OFFSET %s""" % (
                        min_rating, modulo, limit, offset)
        ...
        # we are showing pictures for a DATE selection
            ...
            formatstring = \
                {"wo": "", "year": "%Y", "month": "%Y-%m", "date": "%Y-%m-%d", "": "%Y", "period": "%Y-%m-%d"}[
                    self.args.period]
            if self.args.period == "wo":
                ...
                filelist = MPDB.search_between_dates(DateStart=(self.args.datestart, formatstring),
                                                     DateEnd=(self.args.dateend, formatstring), MinRating=min_rating)
        ...
        # we are showing pictures for a TAG selection
            ...
            filelist = MPDB.filterwizard_result(self.args.kw, self.args.nkw, self.args.matchall, self.args.start,
                                                self.args.end, min_rating)
        ...
        # we are showing pictures for a FOLDER selection
            ...
            filelist = [row for row in MPDB.cur.request(
                """SELECT p.FullPath,f.strFilename FROM Files f, Folders p WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? AND f.idFolder=p.idFolder AND p.ParentFolder in ('%s') ORDER BY ImageDateTime ASC LIMIT %s OFFSET %s""" % (
                    "','".join([str(i) for i in listid]), limit, offset), (min_rating,))]
            ...
            if MPDB.con.get_backend() == "mysql":
                filelist = [row for row in MPDB.cur.request(
                    """SELECT strPath,strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? AND datetime(ImageDateTime) BETWEEN SysDate() - INTERVAL 1 MONTH AND SysDate() ORDER BY ImageDateTime ASC LIMIT %s OFFSET %s""" % (
                        limit, offset), (min_rating,))]
            else:
                filelist = [row for row in MPDB.cur.request(
                    """SELECT strPath,strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? AND datetime(ImageDateTime) BETWEEN datetime('now','-1 months') AND datetime('now') ORDER BY ImageDateTime ASC LIMIT %s OFFSET %s""" % (
                        limit, offset), (min_rating,))]
            ...
            if MPDB.con.get_backend() == "mysql":
                filelist = [row for row in MPDB.cur.request(
                    """SELECT strPath,strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? AND DateAdded>=SysDate() - INTERVAL %s DAY ORDER BY DateAdded ASC LIMIT %s OFFSET %s""" % (
                        numberofdays, limit, offset), (min_rating,))]
            else:
                filelist = [row for row in MPDB.cur.request(
                    """SELECT strPath,strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= ? AND DateAdded >= datetime('now','start of day','-%s days') ORDER BY DateAdded ASC LIMIT %s OFFSET %s""" % (
                        numberofdays, limit, offset), (min_rating,))]

        elif self.args.method == "lastpicsshooted":  # X last pictures shooted __OK
            select = """SELECT strPath,strFilename FROM Files WHERE COALESCE(case ImageRating when '' then '0' else ImageRating end,'0') >= '%s' AND ImageDateTime IS NOT NULL ORDER BY ImageDateTime DESC LIMIT %s""" % (
                min_rating, common.getaddon_setting('lastpicsnumber'))
            filelist = [row for row in MPDB.cur.request(select)]

        elif self.args.method == "videos":  # show all videos __OK
            filelist = [row for row in MPDB.cur.request(
                """SELECT strPath,strFilename FROM Files WHERE ftype="video" ORDER BY ImageDateTime DESC LIMIT %s OFFSET %s""" % (
                    limit, offset))]
        ...
        if self.args.viewmode == "slideshow":
            ...
            command = "SlideShow(%s?action=%%27showpics%%27&method=%%27collection%%27&viewmode=%%27view%%27&page=%%27%%27&collect=%%27%s%%27&name=%%27%s%%27, notrandom) " % (
        ...
        if self.args.viewmode == "zip":
            ...
            if isfile(destination):
                ...
                if not ok:
                    ...
                    common.show_notification(common.getstring(30000), common.getstring(30066), 3000,
                                             join(home, "icon.png"))
            ...
            tar = taropen(destination.encode(sys.getfilesystemencoding()),
                          mode="w:gz")  # open a tar file using gz compression
            error = 0
            ...
            for (path, filename) in filelist:
                ...
                if common.smart_unicode(picture) == common.smart_unicode(
                    ...
                    continue
                pDialog.update(int(100 * (compte / float(len(filelist)))), common.getstring(30067),
                               picture)  # adding picture to the archive
                try:
                    ...
                    error += 1
                    common.log("show_pics >> zip", "Error  %s" % common.smart_unicode(arcname).encode(sys_encoding),
                               xbmc.LOGERROR)
                    print_exc()
        ...
        if self.args.viewmode == "export":
            ...
            for path, filename in filelist:
                ...
                pDialog.update(int(100 * i / len(filelist)), common.getstring(30185) % join(path, filename),
                               dstpath)  # "Copying '%s' to :"
                i = i + 1.0
                if isfile(join(dstpath, filename)):
                    ok = dialog.yesno(common.getstring(30000), common.getstring(30186) % filename, dstpath,
            ...
            xbmc.sleep(1000)
            common.show_notification(common.getstring(30000), common.getstring(30189) % (cpt, dstpath), 3000,
                                     join(home, "icon.png"))
        ...
        for path, filename in filelist:
            ...
            if self.args.method == "collection":
                context.append((common.getstring(30151),
                                "RunPlugin(\"%s?action='delfromcollection'&viewmode='view'&collect='%s'&path='%s'&filename='%s'\")" % (
                                    sys.argv[0],
            ...
            context.append((common.getstring(30060),
                            "RunPlugin(\"%s?action='locate'&filepath='%s'&viewmode='view'\" ,)" % (
                                sys.argv[0], common.quote_param(join(path, filename)))))
            ...
            self.add_picture(filename,
                             ...
                             count=count,
                             contextmenu=context,
                             fanart=xbmcplugin.getSetting(int(sys.argv[1]), 'usepicasfanart') == 'true' and join(path,
                                                                                                                 filename)
                             )
        xbmcplugin.addSortMethod(int(sys.argv[1]), xbmcplugin.SORT_METHOD_DATE)
        xbmcplugin.addSortMethod(
...
if __name__ == "__main__":
    ...
    if not sys.argv[2] or len(sys.argv[2]) == 0:
        ...
        dialog = xbmcgui.Dialog()
        dstpath = dialog.browse(2, common.getstring(
            30071), "files", "", True, False, m.args.filepath)
