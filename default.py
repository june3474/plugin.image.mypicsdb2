...
class Main:
    ...
    def get_args(self):
        ...
        common.log("Main.get_args", "sys.argv[0] = %s" % sys.argv[0])
        common.log("Main.get_args", "sys.argv[2] = %s" % sys.argv[2])
    ...
    def add_directory(self, name, params, action, iconimage=None, fanart=None,
        ...
        try:
            ...
            if path:
                ...
                u = sys.argv[0] + "?" + parameter + "&action=" + action + "&name=" + repr(common.quote_param(name))
    ...
    def add_action(self, name, params, action, iconimage,
                   fanart=None, contextmenu=None, total=0, info="*", replacemenu=True):
        try:
            try:
                parameter = "&".join([param + "=" + repr(common.quote_param(valeur)) for param, valeur in params])
            except Exception as msg:
    ...
    def add_picture(self, picname, picpath, count=0, info="*", fanart=None, contextmenu=None, replacemenu=True):
        ...
        try:
            ...
            try:
                ...
                date = strftime("%d.%m.%Y", strptime(exiftime, "%Y-%m-%d %H:%M:%S")) if exiftime else ""
            except Exception as msg:
            ...
            if count > 0:
                self.add_directory(name="%s (%s %s)" % (childrenfolder, count, common.getstring(30050)),  # libellé
                                   ...
                                   contextmenu=[(common.getstring(30212),
                                                 "Container.Update(\"%s?action='rootfolders'&do='addrootfolder'&addpath='%s'&exclude='1'&viewmode='view'\",)" % (sys.argv[0], common.quote_param(path))), ],
                                   total=len(childrenfolders),
                                   path=path)

        # maintenant, on liste les photos si il y en a, du dossier en cours
        if min_rating > 0:
            _query = "SELECT p.FullPath, f.strFilename FROM Files f, Folders p WHERE f.idFolder=p.idFolder AND "
            _query += "f.idFolder=? AND f.ImageRating > ? order by f.imagedatetime"
            picsfromfolder = [row for row in MPDB.cur.request_with_binds(_query, (self.args.folderid, min_rating,))]
        else:
            _query =  "SELECT p.FullPath, f.strFilename FROM Files f, Folders p WHERE f.idFolder=p.idFolder AND "
            _query += "f.idFolder=? order by f.imagedatetime"
            picsfromfolder = [row for row in MPDB.cur.request_with_binds(_query, (self.args.folderid,))]
            ...
            if len(GlobalFilterTrue) > 0 or len(GlobalFilterFalse) > 0 or start_date != '' or end_date != '':
                xbmc.executebuiltin(
                    "Container.Update(%s?action='showpics'&viewmode='view'&method='wizard'&matchall='%s'&kw='%s'&nkw='%s'&start='%s'&end='%s')" % (
                        sys.argv[0], matchall, common.quote_param(newtagtrue), common.quote_param(newtagfalse), start_date, end_date))
