...
class Main:
    ...
    def get_args(self):
        ...
        self.parm = common.smart_utf8(unquote(sys.argv[2])).replace("\\\\", "\\") #.replace("?&", "&")
        common.log("Main.get_args", "self.parm] = %s"%self.parm, xbmc.LOGINFO)
    ...
    def add_directory(self,name,params,action,iconimage,fanart=None,contextmenu=None,total=0,info="*",replacemenu=True):
        ...
        try:
            try:
                parameter="&".join([param+"="+repr(common.quote_param(valeur)) for param,valeur in params])
            except Exception as msg:
                common.log("Main.add_directory",  "%s - %s"%(Exception,str(msg)), xbmc.LOGERROR )
                parameter=""

            u=sys.argv[0]+"?"+parameter+"&action="+repr(str(action))+"&name="+repr(common.quote_param(name))
    ...
    def show_date(self):
        ...
        if count > 0:
            self.add_directory(name      = common.getstring(30100)%(nameperiode, count), #libellé#"All the period %s (%s pics)"%(self.args.value,MPDB.count_pics_in _period(allperiod,self.args.value)), #libellé
                    ...
                    contextmenu   = [(common.getstring(30152),"RunPlugin(\"%s?action='addfolder'&method='date'&period='%s'&value='%s'&viewmode='scan'\")"%(sys.argv[0],allperiod,self.args.value)),]
                    )
        ...
        for period in listperiod:
            if period:
                if action=="showpics":
                    context = [(common.getstring(30152),"RunPlugin(\"%s?action='addfolder'&method='date'&period='%s'&value='%s'&page=''&viewmode='scan'\")"%(sys.argv[0],nextperiod,period))]
                else:
                    context = [(common.getstring(30152),"RunPlugin(\"%s?action='addfolder'&method='date'&period='%s'&value='%s'&viewmode='scan'\")"%(sys.argv[0],self.args.period,period))]
    ...
    def show_folders(self):
        ...
        for idchildren, childrenfolder in childrenfolders:
            ...
            if count > 0:
                self.add_directory(name      = "%s (%s %s)"%(childrenfolder, count, common.getstring(30050)), #libellé
                        ...
                        contextmenu   = [(common.getstring(30212),"Container.Update(\"%s?action='rootfolders'&do='addrootfolder'&addpath='%s'&exclude='1'&viewmode='view'\",)"%(sys.argv[0],common.quote_param(path)) ),],
        ...
        for path, filename in picsfromfolder:
            ...
            context.append( ( common.getstring(30152),"RunPlugin(\"%s?action='addtocollection'&viewmode='view'&path='%s'&filename='%s'\")"%(sys.argv[0],
    ...
    def show_wizard(self):
        if self.args.wizard == 'dialog':
            ...
            if len(GlobalFilterTrue) > 0 or len(GlobalFilterFalse) > 0 or start_date != '' or end_date != '':
                xbmc.executebuiltin("Container.Update(%s?action='showpics'&viewmode='view'&method='wizard'&matchall='%s'&kw='%s'&nkw='%s'&start='%s'&end='%s')" % ( sys.argv[0], matchall, common.quote_param(newtagtrue), common.quote_param(newtagfalse), start_date, end_date) )
            ...
            for filtername in filterlist:
                ...
                self.add_directory(name      = "%s"%(filtername),
                            ...
                            contextmenu   = [(common.getstring(30152),"RunPlugin(\"%s?action='addfolder'&method='wizard_settings'&filtername='%s'&viewmode='scan'\")"%(sys.argv[0],filtername)),],
                            #contextmenu   = [('','')],
                            total         = total)
            xbmcplugin.addSortMethod(int(sys.argv[1]), xbmcplugin.SORT_METHOD_LABEL)
            xbmcplugin.endOfDirectory(int(sys.argv[1]))
        ...
        for tag, nb in listtags:
            if nb:
                self.add_directory(name      = "%s (%s %s)"%(tag,nb,common.getstring(30050)), #libellé
                            ...
                            action    = "showpics",#action
                            iconimage = join(PIC_PATH,"folder_tags.png"),#icone
                            contextmenu   = [( common.getstring(30152),"RunPlugin(\"%s?action='addfolder'&method='tag'&tag='%s'&tagtype='%s'&viewmode='scan'\")"%(sys.argv[0],common.quote_param(tag),tagtype)),
                                             ( common.getstring(30061),"RunPlugin(\"%s?action='showpics'&method='tag'&page=''&viewmode='zip'&name='%s'&tag='%s'&tagtype='%s'\")"%(sys.argv[0],common.quote_param(tag),common.quote_param(tag),tagtype) ),
                                             ( common.getstring(30062),"RunPlugin(\"%s?action='showpics'&method='tag'&page=''&viewmode='export'&name='%s'&tag='%s'&tagtype='%s'\")"%(sys.argv[0],common.quote_param(tag),common.quote_param(tag),tagtype) )
                                             ],#menucontextuel
                            total = total)#nb total d'éléments
        xbmcplugin.addSortMethod(int(sys.argv[1]), xbmcplugin.SORT_METHOD_LABEL)
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def show_period(self): #TODO finished the datestart and dateend editing
        ...
        if self.args.period=="setperiod":
            ...
            nameddates = [strftime(self.prettydate(common.getstring(30002),strptime(date,"%Y-%m-%d")),strptime(date,"%Y-%m-%d")) for date in dateofpics]
        ...
        for periodname,dbdatestart,dbdateend in MPDB.periods_list():
            ...
            self.add_directory(name      = "%s [COLOR=C0C0C0C0](%s)[/COLOR]"%(periodname,
                                               common.getstring(30113)%(strftime(self.prettydate(common.getstring(30002),strptime(datestart,"%Y-%m-%d")),strptime(datestart,"%Y-%m-%d")),
                                                                    strftime(self.prettydate(common.getstring(30002),strptime(dateend  ,"%Y-%m-%d")),strptime(dateend  ,"%Y-%m-%d"))
                                                                    )), #libellé
                        params    = [("method","date"),("period","period"),("datestart",datestart),("dateend",dateend),("page","1"),("viewmode","view")],#paramètres
                        action    = "showpics",#action
                        iconimage = join(PIC_PATH,"folder_date_ranges.png"),#icone
                        contextmenu   = [ ( common.getstring(30111),"RunPlugin(\"%s?action='removeperiod'&viewmode='view'&periodname='%s'&period='period'\")"%(sys.argv[0],common.quote_param(periodname)) ),
                                          ( common.getstring(30112),"RunPlugin(\"%s?action='renameperiod'&viewmode='view'&periodname='%s'&period='period'\")"%(sys.argv[0],common.quote_param(periodname)) ),
                                          ( common.getstring(30152),"RunPlugin(\"%s?action='addfolder'&method='date'&period='period'&datestart='%s'&dateend='%s'&viewmode='scan'\")"%(sys.argv[0],datestart,dateend))
                                        ] )#menucontextuel
    ...
    def show_collection(self):
            ...
            if ret > -1:
                ...
                if (kb.isConfirmed()):
                    ...
                    if rows != None:
                        ...
                        common.log("show_collection", str(filters[ret]) + " is empty and therefore not created.", xbmc.LOGINFO)
            refresh = True
        ...
        for collection in MPDB.collections_list():
            self.add_action(name      = collection[0],
                        ...
                        contextmenu   = [
                                         (common.getstring(30169),"Container.Update(\"%s?action='showpics'&method='collection'&page=''&viewmode='view'&name='%s'&collect='%s'\")"%(sys.argv[0],common.quote_param(collection[0]),common.quote_param(collection[0])) ),                                         
                                         (common.getstring(30149),"RunPlugin(\"%s?action='collectionaddplaylist'&viewmode='view'&collect='%s'\")"%(sys.argv[0],common.quote_param(collection[0]) ) ),
                                         (common.getstring(30158),"RunPlugin(\"%s?action='removecollection'&viewmode='view'&collect='%s'\")"%(sys.argv[0],common.quote_param(collection[0]) ) ),
                                         (common.getstring(30159),"RunPlugin(\"%s?action='renamecollection'&viewmode='view'&collect='%s'\")"%(sys.argv[0],common.quote_param(collection[0])) ),
                                         (common.getstring(30061),"RunPlugin(\"%s?action='showpics'&method='collection'&page=''&viewmode='zip'&name='%s'&collect='%s'\")"%(sys.argv[0],common.quote_param(collection[0]),common.quote_param(collection[0])) ),
                                         (common.getstring(30062),"RunPlugin(\"%s?action='showpics'&method='collection'&page=''&viewmode='export'&name='%s'&collect='%s'\")"%(sys.argv[0],common.quote_param(collection[0]),common.quote_param(collection[0])) )
                                         ] )#menucontextuel
    ...
    def global_search(self):
        ...
        for tag, _ in listtags:            
            ...
            if compte:
                result = True
                self.add_directory(name      = common.getstring(30116)%(compte,motrecherche,tag ), #files_fields_description.has_key(colname) and files_fields_description[colname] or colname),
                            ...
                            contextmenu   = [(common.getstring(30152),"RunPlugin(\"%s?action='addfolder'&method='search'&field='%s'&searchterm='%s'&viewmode='scan'\")"%(sys.argv[0],tag,motrecherche))])#menucontextuel
        if not result:
            dialog = xbmcgui.Dialog()
                ...
                for source in sources:
                    try:
                        if source.startswith('multipath://'):
                            common.log("Main.show_roots", 'Adding Multipath: "%s"'%unquote(source))
                            newpartialroot = source[12:-1].split('/')
                            for item in newpartialroot:
                                MPDB.add_root_folder(unquote(item),True,True,0)#TODO : traiter le exclude (=0 pour le moment) pour gérer les chemins à exclure
                                common.log("Main.show_roots", 'Multipath addroot for part "%s" done'%unquote(item))
                        else:
            ...
            if str(self.args.exclude)=="1":
                ...
                try:
                    if newroot.startswith('multipath://'):
                        common.log("Main.show_roots", 'Adding Multipath: "%s"'%unquote(newroot))
                        newpartialroot = newroot[12:-1].split('/')
                        for item in newpartialroot:
                            MPDB.add_root_folder(unquote(item),recursive,update,0)#TODO : traiter le exclude (=0 pour le moment) pour gérer les chemins à exclure
                            common.log("Main.show_roots", 'Multipath addroot for part "%s" done'%unquote(item))
                    else:
        ...
        if int(sys.argv[1]) >= 0:
            ...
            for path,recursive,update in includefolders:
                ...
                self.add_action(name      = "[COLOR=FF66CC00][B][ + ][/B][/COLOR] "+path+" [COLOR=FFC0C0C0][recursive="+srec+" , update="+supd+"][/COLOR]",
                            ...
                            iconimage = join(PIC_PATH,"folder_paths.png"),#icone
                            #menucontextuel
                            contextmenu   = [( common.getstring(30206),"Notification(TODO : scan folder,scan this folder now !,3000,%s)"%join(home,"icon.png") ),
                                             ( common.getstring(30207),"Container.Update(\"%s?action='rootfolders'&do='delroot'&delpath='%s'&exclude='1'&viewmode='view'\",)"%(sys.argv[0],common.quote_param(path)))
                                             ]
                            )
    ...
    def show_pics(self):
        ...
        # we are showing pictures for a TAG selection
            ...
            filelist = MPDB.filterwizard_result(self.args.kw, self.args.nkw, self.args.matchall, self.args.start, self.args.end, min_rating)
        ...
        if self.args.viewmode=="zip":
            ...
            if isfile(destination):
                dialog = xbmcgui.Dialog()
                ok = dialog.yesno(common.getstring(30000),common.getstring(30064)%basename(destination),dirname(destination), common.getstring(30065))#Archive already exists, overwrite ?
                if not ok:
        ...
        if self.args.viewmode=="export":
            ...
            if ok:
                dirok=False
                while not dirok:
                    kb = xbmc.Keyboard(self.args.name, common.getstring(30182), False)#Input subfolder name
                    kb.doModal()
            ...
            dialog.browse(2, common.getstring(30188),"files" ,"", True, False, dstpath)#show the folder which contain pictures exported
            return
        ...
        for path,filename in filelist:
            ...
            if self.args.method=="collection":
                context.append( ( common.getstring(30151),"RunPlugin(\"%s?action='delfromcollection'&viewmode='view'&collect='%s'&path='%s'&filename='%s'\")"%(sys.argv[0],
            ...
            #3 - 
            context.append( (common.getstring(30060),"RunPlugin(\"%s?action='locate'&filepath='%s'&viewmode='view'\" ,)"%(sys.argv[0],common.quote_param(join(path,filename)) ) ) )
