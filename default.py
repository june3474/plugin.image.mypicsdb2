...
class Main:
    ...
    def find_folder_thumb(self, path):
        """Get cached thumnails of the given path(folder)

        Args:
            path (str): path to a folder

        Returns (str or None):
            If cached thumbnail exists, path to the thumbnail file, otherwise None
        """
        ...
        prefix = 'image://picturefolder@'
        # kodi adds an extra '/' at the end of the url
        path_quoted = prefix + quote(path, safe='()') + '/'
        crc = self.calc_crc(path_quoted.lower())
        thumb_file = join(thumb_base_folder, crc[2], crc[2:] + '.png')
        common.log("Main.find_folder_thumb", 'folder: "%s"' % path, xbmc.LOGINFO)
        common.log("Main.find_folder_thumb", 'thum_file: "%s"' % thumb_file, xbmc.LOGINFO)
    ...
    def add_directory(self, name, params, action, iconimage=None, fanart=None,
        ...
        try:
            ...
            if path:
                # By using path instead of plugin call, we can use the default folder icon generated by Kodi.
                u = path
            else:
            ...
            if iconimage:
                common.log("Main.add_directory", 'iconimage: "%s"' % iconimage, xbmc.LOGINFO)
        ...
        for idchildren, childrenfolder in childrenfolders:
            ...
            if count > 0:
                ...
                self.add_directory(name=name, 
                                   ...
                                   iconimage=self.find_folder_thumb(path), # icone
